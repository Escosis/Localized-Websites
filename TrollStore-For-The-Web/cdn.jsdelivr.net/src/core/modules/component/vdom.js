...
const createCustomComponent = ({ f7, treeNode, vnode, data }) => {
  const component =
    typeof treeNode.type === "function"
      ? treeNode.type
      : customComponents[treeNode.type];
  f7.component
...
const isCustomComponent = (treeNodeType) => {
  return (
    ...
    (treeNodeType &&
      treeNodeType.indexOf("-") > 0 &&
      customComponents[treeNodeType])
  );
};
  ...
  Object.keys(treeNode.props).forEach((attrName) => {
    ...
    if (PROPS_ATTRS.indexOf(attrName) >= 0) {
      ...
      let eventName =
        attrName.indexOf("@") === 0
          ? attrName.substr(1)
          : eventNameToColonCase(attrName.substr(2));
      let stop = false;
      let prevent = false;
      let once = false;
...
const getSlots = (treeNode, component, f7, initial) => {
  ...
  return slotNodes.map((subTreeNode) =>
    treeNodeToVNode(subTreeNode, component, f7, initial)
  );
};
