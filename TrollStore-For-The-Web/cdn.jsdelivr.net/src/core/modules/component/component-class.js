...
const types = [
  {
    ...
    update: (i, o) =>
      [o].filter(Array.isArray).find(() => ((i.length = 0), i.push(...o))),
    insert: (i, x, o = []) => i.splice(Math.max(x, 0), 0, ...[o].flat()),
    replace: (i, x, o = []) =>
      i.splice(Math.max(x, 0), Math.min(++x, 1), ...[o].flat()),
    ...
    fromTo: (i, a, b = a) =>
      i.splice(Math.max(b, 0), 0, ...i.splice(Math.max(a, 0), 1)),
    ...
    return (o) => {
      const obj = [o].reduce(
        (t, _i, _x, _a, i = t.init(_i)) => ({
          ...
          async: (f = () => Promise.reject(i)) =>
            f(i).then(() => this.update()),
        }),
