...
function backward(router, el, backwardOptions) {
  ...
  const currentIsMaster =
    masterDetailEnabled && $oldPage.hasClass("page-master");
  ...
  if (masterDetailEnabled) {
    ...
    if (
      ...
      isDetail =
        options.route.route.masterRoute.path ===
        masterPageEl.f7Page.route.route.path;
    }
  }
  ...
  $newPage
    .addClass(
      `page-${initialPreload ? "current" : "previous"}${
        isMaster ? " page-master" : ""
  ...
  router.emit(
    "pagePosition",
    $newPage[0],
    initialPreload ? "current" : "previous"
  );
  ...
  if (dynamicNavbar && $newNavbarEl.length > 0) {
    $newNavbarEl
      .addClass(
        `navbar-${initialPreload ? "current" : "previous"}${
          isMaster ? " navbar-master" : ""
  ...
  if (options.force) {
    if ($oldPage.prev(".page-previous").length >= 0) {
      if (router.history.indexOf(options.route.url) >= 0) {
        backIndex =
          router.history.length - router.history.indexOf(options.route.url) - 1;
        ...
        router.propsHistory[router.propsHistory.length - 2] =
          options.props || {};
      } else {
  ...
  if (options.preload) {
    ...
    if (isMaster) {
      ...
      if (dynamicNavbar) {
        $(app.navbar.getElByPage($newPage)).removeClass(
  ...
  if (
    options.animate &&
    !(currentIsMaster && app.width >= router.params.masterDetailBreakpoint)
  ) {
...
function back(...args) {
  ...
  if (currentRouteIsModal && !navigateOptions.preload) {
    ...
    if (previousRoute && modalToClose) {
      const isBrokenBrowserHistory =
        device.ie || device.edge || (device.firefox && !device.ios);
      const needHistoryBack =
        router.params.browserHistory &&
        navigateOptions.browserHistory !== false;
      ...
      if (
        needHistoryBack &&
        !isBrokenBrowserHistory &&
        !currentRouteWithoutBrowserHistory
      ) {
        History.back();
      }
  ...
  if (router.params.masterDetailBreakpoint > 0) {
    ...
    if ($previousMaster.length) {
      ...
      const expectedPreviousPageRoute = router.findMatchingRoute(
        expectedPreviousPageUrl
      );
      if (
        expectedPreviousPageRoute &&
        $previousMaster[0].f7Page &&
        expectedPreviousPageRoute.route ===
          $previousMaster[0].f7Page.route.route
      ) {
