...
function modalLoad(modalType, route, loadOptions = {}, direction) {
  ...
  const routeCallback = (modal, name) => {
    ...
    if (name === "open") {
      callback =
        on.modalOpen || once.modalOpen || on.panelOpen || once.panelOpen;
    }
    if (name === "close") {
      callback =
        on.modalClose || once.modalClose || on.panelClose || once.panelClose;
    }
    if (name === "closed") {
      callback =
        on.modalClosed ||
  ...
  function onModalLoaded() {
    ...
    modal.on(`${modalOrPanel}Closed`, () => {
      modal.$el.trigger(
        `${modalType.toLowerCase()}:beforeremove`,
        route,
        modal
      );
      modal.emit(
    ...
    // Open
    modal.open(
      options.animate === false || options.animate === true
        ? options.animate
        : undefined
    );
  }
