...
function SwipeBack(r) {
  ...
  const paramsSwipeBackAnimateShadow =
    params[`${app.theme}SwipeBackAnimateShadow`];
  const paramsSwipeBackAnimateOpacity =
    params[`${app.theme}SwipeBackAnimateOpacity`];
  ...
  function animatableNavElements() {
    ...
    const currentNavIsCollapsed = $currentNavbarEl.hasClass(
    ...
    const previousNavIsCollapsed = $previousNavbarEl.hasClass(
      "navbar-large-collapsed"
    );
    ...
    if (params.iosAnimateNavbarBackIcon) {
      ...
      if (activeNavBackIconText.length) {
        $previousNavElements.each((el) => {
          ...
          el.f7NavbarLeftOffset +=
            activeNavBackIconText.prev(".icon")[0].offsetWidth;
        });
      }
    }
    $currentNavElements.each((navEl) => {
        ...
        if ($navEl.hasClass("title-large")) {
          ...
          $navEl.find(".title-large-text").each((subNavEl) => {
            els.push({
              el: subNavEl,
              transform: (progress) =>
                `translateX(${progress * 100 * inverter}%)`,
            });
          });
          return;
        }
      ...
      if (isBg) {
        ...
        if (!fromLarge && !toLarge) {
          if (currentNavIsCollapsed) {
            ...
            el.transform = (progress) =>
              `translateX(${100 * progress * inverter}%)`;
          }
        }
        if (!fromLarge && toLarge) {
          el.className = "ios-swipeback-navbar-bg-large";
      ...
      const isSliding =
        $navEl.hasClass("sliding") ||
        $navEl.parents(".navbar-inner.sliding").length;
      if (els.indexOf(el) < 0) els.push(el);
      ...
      if (isSliding) {
        ...
        if (
          isLeft &&
          activeNavBackIconText.length &&
          params.iosAnimateNavbarBackIcon
        ) {
        ...
        transformTarget.transform = (progress) => {
          let activeNavTranslate =
            progress * transformTarget.el.f7NavbarRightOffset;
    ...
    $previousNavElements.each((navEl) => {
      ...
      if (isBg) {
        ...
        if (!fromLarge && !toLarge) {
          if (previousNavIsCollapsed) {
            ...
            el.transform = (progress) =>
              `translateX(${(-100 + 100 * progress) * inverter}%)`;
          }
        }
        if (!fromLarge && toLarge) {
          el.transform = (progress) =>
      ...
      const isSliding =
        $navEl.hasClass("sliding") ||
        $previousNavbarEl.children(".navbar-inner.sliding").length;
      if (els.indexOf(el) < 0) els.push(el);
      if (!isSubnavbar || (isSubnavbar && !isSliding)) {
      ...
      if (isSliding) {
        ...
        if (
          isLeft &&
          previousNavBackIconText.length &&
          params.iosAnimateNavbarBackIcon
        ) {
        ...
        transformTarget.transform = (progress) => {
          let previousNavTranslate =
            transformTarget.el.f7NavbarLeftOffset * (1 - progress);
          if (device.pixelRatio === 1)
  ...
  function setAnimatableNavElements({
    ...
    if (
      reflow &&
      animatableNavEls.length &&
      animatableNavEls[0] &&
      animatableNavEls[0].el
    ) {
  ...
  function handleTouchStart(e) {
    ...
    if (
      ...
      return;
    if ($(e.target).closest(".range-slider, .calendar-months").length > 0)
      return;
    if (
  ...
  function handleTouchMove(e) {
    ...
    if (!isMoved) {
      ...
      let notFromBorder =
        touchesStart.x - $el.offset().left > paramsSwipeBackActiveArea;
      ...
      if (app.rtl) {
        notFromBorder =
          ...
          $el.offset().left -
            $el[0].scrollLeft +
            (viewContainerWidth - paramsSwipeBackActiveArea);
      } else {
        notFromBorder =
          touchesStart.x - $el.offset().left > paramsSwipeBackActiveArea;
      }
      ...
      if (dynamicNavbar) {
        ...
        if ($previousNavbarEl.length > 1) {
          $previousNavbarEl = $previousNavbarEl.eq(
            $previousNavbarEl.length - 1
          );
        }
    ...
    // Touches diff
    touchesDiff =
      (pageX - touchesStart.x - paramsSwipeBackThreshold) * inverter;
    if (touchesDiff < 0) touchesDiff = 0;
    const percentage = Math.min(
      Math.max(touchesDiff / viewContainerWidth, 0),
      1
    );
    ...
    $([$currentPageEl[0], $previousPageEl[0]]).addClass(
  ...
  function handleTouchEnd(e) {
    ...
    if (
      ...
      $currentPageEl
        ...
        .addClass(
          `page-next${app.theme !== "ios" ? " page-next-on-right" : ""}`
        );
      $previousPageEl
