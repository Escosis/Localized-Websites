...
class Router extends Framework7Class {
  ...
  animatableNavElements(
    ...
    function animatableNavEl($el, $navbarInner) {
      const isSliding =
        $el.hasClass("sliding") || $navbarInner.hasClass("sliding");
    ...
    if (dynamicNavbar) {
      ...
      $newNavbarEl
        ...
        .each((navEl) => {
          ...
          newNavEls.push(
            animatableNavEl($navEl, $newNavbarEl.children(".navbar-inner"))
          );
        });
      if (
        !(
        ...
        $oldNavbarEl
          ...
          .each((navEl) => {
            ...
            oldNavEls.push(
              animatableNavEl($navEl, $oldNavbarEl.children(".navbar-inner"))
            );
          });
      }
  ...
  animate(
    ...
    if (ios && dynamicNavbar) {
      ...
      if (!betweenMasterAndDetail) {
        ...
        fromLarge =
          oldIsLarge && !$oldNavbarEl.hasClass("navbar-large-collapsed");
        toLarge =
          newIsLarge && !$newNavbarEl.hasClass("navbar-large-collapsed");
    ...
    function animateNavbars(progress) {
      ...
      newNavEls.forEach((navEl) => {
        const $el = navEl.$el;
        const offset =
          direction === "forward" ? navEl.rightOffset : navEl.leftOffset;
        if (navEl.isSliding) {
      ...
      oldNavEls.forEach((navEl) => {
        const $el = navEl.$el;
        const offset =
          direction === "forward" ? navEl.leftOffset : navEl.rightOffset;
        if (navEl.isSliding) {
    ...
    routes.forEach((route) => {
      ...
      if ("routes" in route) {
        const mergedPathsRoutes = route.routes.map((childRoute) => {
          ...
          cRoute.path = `${route.path}/${cRoute.path}`
            .replace("///", "/")
            .replace("//", "/");
          return cRoute;
        });
    ...
    flattenedRoutes.forEach((route) => {
      if (
        ...
        if (
          ...
          Object.keys(router.currentRoute.params).length > 0
        ) {
  ...
  pageCallback(callback, pageEl, navbarEl, from, to, options = {}, pageFromEl) {
    ...
    const camelName = `page${
      callback[0].toUpperCase() + callback.slice(1, callback.length)
    }`;
      ...
      if ($pageEl[0].f7RouteEventsOn) {
        Object.keys($pageEl[0].f7RouteEventsOn).forEach((eventName) => {
          $pageEl.off(
            ...
            $pageEl[0].f7RouteEventsOn[eventName]
          );
        });
      }
  ...
  getInitialUrl() {
    ...
    let historyRestored;
    const { browserHistory, browserHistoryOnLoad, browserHistorySeparator } =
      router.params;
    ...
    if (!browserHistory || !browserHistoryOnLoad) {
      ...
      if (
        browserHistorySeparator.length > 0 &&
        documentUrl.indexOf(browserHistorySeparator) >= 0
      ) {
        initialUrl = documentUrl.substring(
          documentUrl.indexOf(browserHistorySeparator) +
            browserHistorySeparator.length
        );
      } else {
        initialUrl = documentUrl;
      }
      ...
      if (router.history.indexOf(initialUrl) >= 0) {
        router.history = router.history.slice(
          0,
          router.history.indexOf(initialUrl) + 1
        );
        ...
        router.history = [
          documentUrl.split(browserHistorySeparator)[0] || "/",
          initialUrl,
        ];
      }
  ...
  init() {
    ...
    if (router.history.length > 1) {
      // Will load page
      const initUrl = browserHistoryInitialMatch
        ? initialUrl
        : router.history[0];
    ...
    if (
      router.$el.children(".page").length === 0 &&
      initialUrl &&
