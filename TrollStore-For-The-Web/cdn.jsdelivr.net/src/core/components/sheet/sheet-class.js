  ...
  constructor(app, params) {
    ...
    if (sheet.params.backdrop && sheet.params.backdropEl) {
      ...
      if (sheet.params.backdropUnique) {
        $backdropEl = $(
          '<div class="sheet-backdrop sheet-backdrop-unique"></div>'
        );
    ...
    function scrollToElementOnOpen() {
      ...
      const pageHeight =
        $pageContentEl[0].offsetHeight - paddingTop - $el.height();
      const pageScrollHeight =
        $pageContentEl[0].scrollHeight - paddingTop - $el.height();
      const pageScroll = $pageContentEl.scrollTop();

      let newPaddingBottom;

      const scrollElTop =
        $scrollEl.offset().top - paddingTop + $scrollEl[0].offsetHeight;
    ...
    const useBreakpoints =
      sheet.params.breakpoints && sheet.params.breakpoints.length > 0;
    let isTouched = false;
    let startTouch;
    let currentTouch;
    let isScrolling;
    ...
    function handleTouchStart(e) {
      if (
        isTouched ||
        !(sheet.params.swipeToClose || sheet.params.swipeToStep) ||
        !e.isTrusted
      )
        return;
      if (
    ...
    function handleTouchMove(e) {
      ...
      if (typeof isScrolling === "undefined") {
        isScrolling = !!(
          isScrolling ||
          Math.abs(currentTouch.x - startTouch.x) >
            Math.abs(currentTouch.y - startTouch.y)
        );
      }
      if (isScrolling) {
        isTouched = false;
      ...
      if (!isMoved) {
        ...
        if (isTopSheetModal) {
          minTranslate = sheet.params.swipeToClose
            ? -sheetElOffsetHeight
            : -swipeStepTranslate;
          maxTranslate = 0;
        } else {
      ...
      if (sheet.push && pushOffset && !useBreakpoints) {
        let progress =
          (currentTranslate - startTranslate) / sheetElOffsetHeight;
        if (sheet.params.swipeToStep) {
          if (isTopSheetModal) {
            progress = currentTranslate / swipeStepTranslate;
          } else {
            progress =
              1 - (swipeStepTranslate - currentTranslate) / swipeStepTranslate;
          }
        }
        progress = Math.abs(progress);
        progress = Math.min(Math.max(progress, 0), 1);
        ...
        if (sheet.params.swipeToStep) {
          $pushViewEl.css(
            "border-radius",
            `${pushBorderRadius * pushProgress}px`
          );
        }
      }
      ...
      if (sheet.params.swipeToStep) {
        ...
        if (isTopSheetModal) {
          ...
          progress =
            (swipeStepTranslate - currentTranslate) / swipeStepTranslate;
        }
        progress = Math.min(Math.max(progress, 0), 1);
    ...
    function handleTouchEnd() {
      ...
      if (!sheet.params.swipeToStep && !useBreakpoints) {
        ...
        if (
          (timeDiff < 300 && diff > 20) ||
          (timeDiff >= 300 && diff > sheetElOffsetHeight / 2)
        ) {
          sheet.close();
        }
        return;
      ...
      if (timeDiff < 300 && diff > 10 && useBreakpoints) {
        ...
        if (
          ...
          typeof currentBreakpointIndex !== "undefined"
        ) {
      ...
      if (timeDiff >= 300 && useBreakpoints) {
        ...
        const closestTranslate = allBreakpoints.reduce((prev, curr) => {
          return Math.abs(curr - currentTranslate) <
            Math.abs(prev - currentTranslate)
            ? curr
            : prev;
        });
    ...
    const setPushBreakpoint = (breakpoint) => {
      ...
      if (breakpoint >= pushBreakpoint) {
        ...
        if (breakpoint <= pushTransparentBreakpoint) {
          ...
          const progress =
            (breakpoint - pushTransparentBreakpoint) /
            (pushBreakpoint - pushTransparentBreakpoint);
          sheet.$htmlEl
            .addClass("with-modal-sheet-push")
            .removeClass("with-modal-sheet-push-closing");
          $pushViewEl.transition(0).forEach((el) => {
            el.style.setProperty(
              ...
              `translate3d(0,0,0) scale(${
                1 - (1 - pushViewScale(pushOffset)) * progress
              })`,
              "important"
            );
          });
    ...
    const setBackdropBreakpoint = (breakpoint) => {
      ...
      if (breakpoint >= backdropBreakpoint) {
        ...
        const backdropTransparentBreakpoint =
          backdropBreakpoints[
            backdropBreakpoints.indexOf(backdropBreakpoint) - 1
          ];
        if (breakpoint <= backdropTransparentBreakpoint) {
          ...
          $backdropEl
            .transition(0)
            .css({ opacity: progress, pointerEvents: "auto" });
        }
      }
    };
    ...
    sheet.calcBreakpoints = () => {
      ...
      sheet.params.breakpoints.forEach((ratio) => {
        breakpointsTranslate.push(
          (fullSize - fullSize * ratio) * (isTopSheetModal ? -1 : 1)
        );
      });
    };
    ...
    if (
      sheet.params.swipeToClose ||
      sheet.params.swipeToStep ||
      useBreakpoints
    ) {
    ...
    sheet.on("opened", () => {
      if (
        sheet.params.closeByOutsideClick ||
        sheet.params.closeByBackdropClick
      ) {
