...
class SmartSelect extends Framework7Class {
      ...
      if (inputEl.type === "checkbox") {
        for (let i = 0; i < ss.selectEl.options.length; i += 1) {
          ...
          if (optionEl.selected) {
            ...
            text =
              displayAs && typeof displayAs !== "undefined"
                ? displayAs
        ...
        if (!optionEl) {
          optionEl = ss.$selectEl
            .find("option")
            .filter((optEl) => optEl.value === value)[0];
        }
        displayAs = optionEl.dataset
          ? optionEl.dataset.displayAs
          : $(optionEl).data("display-as");
        text =
          displayAs && typeof displayAs !== "undefined"
            ? displayAs
            : optionEl.textContent;
  ...
  getItemsData() {
    ...
    ss.$selectEl.find("option").each((optionEl) => {
      ...
      const optionIconIos =
        theme === "ios" &&
        (optionData.optionIconIos || ss.params.optionIconIos);
      const optionIconMd =
        theme === "md" && (optionData.optionIconMd || ss.params.optionIconMd);
      const optionInputIconPosition =
        optionData.inputIconPosition || ss.params.inputIconPosition || "";

      const optionHasMedia =
        optionImage || optionIcon || optionIconIos || optionIconMd;
  ...
  renderItem(item, index) {
    ...
    if (item.isLabel) {
      ...
      if (ss.params.virtualList) {
        ...
        selected = ss.multiple
          ? ssValue.indexOf(item.value) >= 0
          : ssValue === item.value;
        if (ss.multiple) {
          disabled =
            ss.multiple &&
            !selected &&
            ssValue.length === parseInt(ss.maxLength, 10);
        }
      }
        ...
        <li class={`${item.className || ""}${disabled ? " disabled" : ""}`}>
          <label
            class={`item-${item.inputType} ${
              item.inputIconPosition
                ? `item-${item.inputType}-icon-${item.inputIconPosition}`
                : ""
            } item-content`}
          >
            <input
            ...
            <div class="item-inner">
              <div
                class={`item-title${
                  item.color ? ` text-color-${item.color}` : ""
                }`}
              >
                {item.text}
              </div>
        ...
        <div
          class={`navbar ${
            ss.params.navbarColorTheme
              ? `color-${ss.params.navbarColorTheme}`
              : ""
          }`}
        >
          ...
          >
            ...
            {ss.params.searchbar && (
              <div class="subnavbar">{ss.renderSearchbar()}</div>
            )}
          </div>
        </div>
          ...
          <div
            class={`list list-outline-ios list-strong-ios list-dividers-ios smart-select-list-${
              ...
              ss.params.formColorTheme
                ? `color-${ss.params.formColorTheme}`
                : ""
            }`}
          >
        ...
        <div class="view">
          <div
            class={`page smart-select-page ${
              ss.params.searchbar ? "page-with-subnavbar" : ""
            }`}
            data-name="smart-select-page"
          >
            <div
  ...
  onOpen(type, containerEl) {
    ...
    if (ss.params.virtualList) {
      ss.vl = app.virtualList.create({
        ...
        searchByItem(query, item) {
          if (
            item.text &&
            removeDiacritics(item.text)
              .toLowerCase()
              .indexOf(query.trim().toLowerCase()) >= 0
          )
            return true;
          return false;
        },
    ...
    if (ss.params.searchbar) {
      ...
      if (type === "page" && app.theme === "ios") {
        $searchbarEl = $(app.navbar.getElByPage($containerEl)).find(
          ".searchbar"
        );
      }
