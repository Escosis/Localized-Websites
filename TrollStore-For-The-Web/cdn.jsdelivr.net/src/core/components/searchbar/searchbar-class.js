...
class Searchbar extends FrameworkClass {
  constructor(app, params = {}) {
    ...
    const expandable =
      sb.params.expandable || $el.hasClass("searchbar-expandable");
    ...
    if (sb.params.backdrop) {
      ...
      if ($backdropEl.length === 0) {
        ...
        if ($pageEl && $pageEl.length) {
          if (
            $el.parents($pageEl).length > 0 &&
            $navbarEl &&
            $el.parents($navbarEl).length === 0
          ) {
    ...
    extend(sb, {
      ...
      query: "",
      isVirtualList:
        $searchContainer && $searchContainer.hasClass("virtual-list"),
    ...
    function onPageBeforeIn() {
      ...
      if (sb.enabled) {
        ...
        if (sb.expandable) {
          sb.$el
            .parents(".navbar")
            .addClass("with-searchbar-expandable-enabled-no-transition");
        }
      }
    }
  ...
  enable(setFocus) {
    ...
    function enable() {
      if (
        sb.$backdropEl &&
        ((sb.$searchContainer && sb.$searchContainer.length) ||
          sb.params.customSearch) &&
      ...
      if (
        !sb.$disableButtonEl ||
        (sb.$disableButtonEl && sb.$disableButtonEl.length === 0)
      ) {
    ...
    if (isIos) {
      ...
      if (app.theme === "md" && sb.expandable) {
        sb.$el
          .parents(".page, .view, .navbar-inner, .navbar")
          .scrollLeft(app.rtl ? 100 : 0);
      }
      enable();
    }
    return sb;
  ...
  disable() {
    ...
    sb.$el.removeClass(
      "searchbar-enabled searchbar-focused searchbar-enabled-no-disable-button"
    );
    if (sb.expandable) {
      const $navbarEl = sb.$el.parents(".navbar");
  ...
  search(query, internal) {
    ...
    if (isVirtualList) {
      ...
      if (sb.virtualList.params.searchAll) {
        foundItems =
          sb.virtualList.params.searchAll(vlQuery, sb.virtualList.items) || [];
      } else if (sb.virtualList.params.searchByItem) {
        for (let i = 0; i < sb.virtualList.items.length; i += 1) {
          if (
            sb.virtualList.params.searchByItem(
              vlQuery,
              sb.virtualList.items[i],
              i
            )
          ) {
            foundItems.push(i);
      ...
      $searchContainer
        ...
        .each((itemEl) => {
          const $itemEl = $(itemEl);
          let compareWithText = [];
          let $searchIn = sb.params.searchIn
            ? $itemEl.find(sb.params.searchIn)
            : $itemEl;
          ...
          if (
            ...
            !(sb.params.ignore && $itemEl.is(sb.params.ignore))
          ) {
