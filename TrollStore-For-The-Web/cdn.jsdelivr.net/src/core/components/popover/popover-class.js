...
class Popover extends Modal {
  constructor(app, params) {
    ...
    const forceBackdropUnique =
      ...
      app.$el
        .find(".popover.modal-in")
        .filter((anotherPopoverEl) => anotherPopoverEl !== $el[0]).length > 0;
    if (popover.params.backdrop && popover.params.backdropEl) {
      ...
      if (popover.params.backdropUnique || forceBackdropUnique) {
        $backdropEl = $(
          '<div class="popover-backdrop popover-backdrop-unique"></div>'
        );
    ...
    function handleClick(e) {
      ...
      if ($target.closest(popover.el).length === 0) {
        if (
          ...
          popover.params.closeByOutsideClick &&
          touchStartTarget === target
        ) {
          const isAnotherPopoverBackdrop =
            ($target.hasClass("popover-backdrop-unique") &&
              target.f7PopoverRef !== popover) ||
            ($target.hasClass("popover-backdrop") &&
              target !== popover.backdropEl);
          const isAnotherPopoverTarget =
            target.closest(".popover") &&
            target.closest(".popover") !== popover.$el[0];
  ...
  resize() {
    ...
    const forcedPosition =
      verticalPosition === "auto" ? false : verticalPosition;
    ...
    if (
      ...
      (!forcedPosition &&
        height + arrowSize < app.height - targetOffsetTop - targetHeight)
    ) {
      // On bottom
      position = "bottom";
    ...
    if (position === "top" || position === "bottom") {
      ...
      if (hasArrow) {
        ...
        arrowLeft = Math.max(
          Math.min(arrowLeft, width - arrowSize * 2 - arrowMin),
          arrowMin
        );
      ...
      if (
        left < 5 ||
        left + width + safeAreaRight > app.width ||
        left < safeAreaLeft
      ) {
      ...
      if (hasArrow) {
        ...
        arrowTop = Math.max(
          Math.min(arrowTop, height - arrowSize * 2 - arrowMin),
          arrowMin
        );
