...
class DataTable extends Framework7Class {
    ...
    table.$el.find("tbody td:not(.checkbox-cell)").each((el) => {
      ...
      if (!collapsibleTitle && collapsibleTitle !== "") {
        $el.attr(
          ...
          table.$el.find("thead th").eq(elIndex).text()
        );
      }
    });
  }
  ...
  checkSelectedHeader() {
    ...
    if (table.$headerEl.length > 0 && table.$headerSelectedEl.length > 0) {
      const checkedItems = table.$el.find(
        "tbody .checkbox-cell input:checked"
      ).length;
      table.$el[checkedItems > 0 ? "addClass" : "removeClass"](
        "data-table-has-checked"
      );
      table.$headerSelectedEl
        .find(".data-table-selected-count")
        .text(checkedItems);
    }
  }
