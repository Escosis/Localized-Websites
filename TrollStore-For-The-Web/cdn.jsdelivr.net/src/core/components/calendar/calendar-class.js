...
class Calendar extends Framework7Class {
  constructor(app, params = {}) {
    ...
    const timeFormatCheckDate = calendar
      .timeSelectorFormatter(new Date())
      .toLowerCase();
    ...
    // Auto names
    let { monthNames, monthNamesShort, dayNames, dayNamesShort } =
      calendar.params;
    const {
      monthNamesIntl,
      monthNamesShortIntl,
    ...
    function onHtmlClick(e) {
      ...
      if (
        calendar.monthPickerPopover ||
        calendar.yearPickerPopover ||
        calendar.timePickerPopover
      )
        return;
      if ($inputEl && $inputEl.length > 0) {
        ...
        $(e.target).closest(".sheet-modal, .calendar-modal").length === 0
      ) {
        calendar.close();
      }
    }
    ...
    calendar.attachCalendarEvents = function attachCalendarEvents() {
      ...
      function handleTouchMove(e) {
        ...
        currentTranslate =
          (calendar.monthsTranslate * calendar.inverter + percentage) * 100;

        // Transform wrapper
        $wrapperEl.transform(
          `translate3d(${isH ? currentTranslate : 0}%, ${
            isH ? 0 : currentTranslate
          }%, 0)`
        );
      }
        ...
        if (
          ...
          if (calendar.hasTimePicker) {
            if (calendar.value && calendar.value[0]) {
              valueToAdd.setHours(
                calendar.value[0].getHours(),
                calendar.value[0].getMinutes()
              );
            } else {
              valueToAdd.setHours(
                new Date().getHours(),
  ...
  get view() {
    ...
    if (params.view) {
      ...
      view =
        $inputEl.parents(".view").length && $inputEl.parents(".view")[0].f7View;
    }
  ...
  addValue(newValue) {
    const calendar = this;
    const { multiple, rangePicker, rangePickerMinDays, rangePickerMaxDays } =
      calendar.params;
    if (multiple) {
      ...
      for (let i = 0; i < calendar.value.length; i += 1) {
        if (
          new Date(newValue).getTime() === new Date(calendar.value[i]).getTime()
        ) {
          inValuesIndex = i;
        }
      }
  ...
  onMonthChangeEnd(dir, rebuildBoth) {
    ...
    if (!rebuildBoth) {
      currentMonthHtml = calendar.renderMonth(
        new Date(currentYear, currentMonth),
        dir
      );
    } else {
      $wrapperEl.find(".calendar-month-next, .calendar-month-prev").remove();
      prevMonthHtml = calendar.renderMonth(
        new Date(currentYear, currentMonth),
        "prev"
      );
      nextMonthHtml = calendar.renderMonth(
        new Date(currentYear, currentMonth),
        "next"
      );
    }
  ...
  setMonthsTranslate(translate) {
    ...
    $months
      .eq(0)
      .transform(
        `translate3d(${isH ? prevMonthTranslate : 0}%, ${
          isH ? 0 : prevMonthTranslate
        }%, 0)`
      )
    ...
    $months
      .eq(1)
      .transform(
        `translate3d(${isH ? currentMonthTranslate : 0}%, ${
          isH ? 0 : currentMonthTranslate
        }%, 0)`
      )
    ...
    $months
      .eq(2)
      .transform(
        `translate3d(${isH ? nextMonthTranslate : 0}%, ${
          isH ? 0 : nextMonthTranslate
        }%, 0)`
      )
  ...
  nextMonth(transition) {
    ...
    $wrapperEl
      ...
      .transform(
        `translate3d(${isH ? translate : 0}%, ${isH ? 0 : translate}%, 0)`
      );
  ...
  setYearMonth(year, month, transition) {
    ...
    if (targetDate > currentDate) {
      ...
      calendar.$months
        ...
        .transform(
          `translate3d(${isH ? monthTranslate : 0}%, ${
            isH ? 0 : monthTranslate
          }%, 0)`
        )
        .addClass("calendar-month-next");
    } else {
    ...
    $wrapperEl
      .transition(transition)
      .transform(
        `translate3d(${isH ? wrapperTranslate : 0}%, ${
          isH ? 0 : wrapperTranslate
        }%, 0)`
      );
    if (transitionEndCallback) {
      $wrapperEl.transitionEnd(() => {
  ...
  renderMonth(d, offset) {
    ...
    let localeMonth = calendar.monthNames.indexOf(
      calendar.monthFormatter(date)
    );
        ...
        if (hasEvents) {
          ...
          if (Array.isArray(params.events)) {
            ...
            params.events.forEach((ev) => {
              ...
              if (
                eventDots.indexOf(color) < 0 &&
                calendar.dateInRange(dayDate, ev)
              ) {
  ...
  renderWeekHeader() {
    ...
    for (let i = 0; i < 7; i += 1) {
      const dayIndex =
        i + params.firstDay > 6 ? i - 7 + params.firstDay : i + params.firstDay;
  ...
  renderInline() {
    const calendar = this;
    const { cssClass, toolbar, header, footer, rangePicker, weekHeader } =
      calendar.params;
    const { value, hasTimePicker } = calendar;
    const date =
      value && value.length ? value[0] : new Date().setHours(0, 0, 0);
    return (
      <div
        class={`calendar calendar-inline ${
          rangePicker ? "calendar-range" : ""
        } ${cssClass || ""}`}
      >
  ...
  renderSheet() {
    ...
    return (
      <div
        class={`sheet-modal calendar calendar-sheet ${
          rangePicker ? "calendar-range" : ""
        } ${cssClass || ""}`}
      >
        {header && calendar.renderHeader()}
    ...
    return (
      <div class="popover calendar-popover">
        <div class="popover-inner">
          <div
            class={`calendar ${rangePicker ? "calendar-range" : ""} ${
              cssClass || ""
            }`}
          >
            {header && calendar.renderHeader()}
  ...
  openMonthPicker() {
    ...
    calendar.monthPickerInstance = app.picker.create({
      containerEl: calendar.monthPickerPopover.$el.find(
  ...
  openYearPicker() {
    ...
    let yearMin =
      calendar.params.yearPickerMin || new Date().getFullYear() - 100;
    if (calendar.params.minDate) {
      yearMin = Math.max(
        yearMin,
        new Date(calendar.params.minDate).getFullYear()
      );
    }
    let yearMax =
      calendar.params.yearPickerMax || new Date().getFullYear() + 100;
    if (calendar.params.maxDate) {
      yearMax = Math.min(
        yearMax,
        new Date(calendar.params.maxDate).getFullYear()
      );
    }
    const years = [];
    ...
    const modalParams = {
      ...
      on: {
        open() {
          ...
          calendar.$el =
            modalType === "popover" ? modal.$el.find(".calendar") : modal.$el;
          calendar.$wrapperEl = calendar.$el.find(".calendar-months-wrapper");
          calendar.$months = calendar.$wrapperEl.find(".calendar-month");
          calendar.$el[0].f7Calendar = calendar;
