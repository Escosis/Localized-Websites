...
const Fab = {
  morphOpen(fabEl, targetEl) {
    ...
    let borderRadius = Math.ceil(
      parseInt(target.borderRadius, 10) / Math.max(scaleX, scaleY)
    );
    ...
    $fabEl.transitionEnd(() => {
      ...
      nextFrame(() => {
        ...
        $fabEl
          .transform(
            `translate3d(${-diffX}px, ${-diffY}px, 0) scale(${scaleX}, ${scaleY})`
          )
      ...
      if ($targetEl.parents(".page-content").length > 0) {
        $targetEl
          ...
          .on("scroll", $fabEl[0].f7FabMorphResizeHandler);
      }
    });
  },
  morphClose(fabEl) {
    ...
    if ($targetEl.parents(".page-content").length > 0) {
      $targetEl
        .parents(".page-content")
        .off("scroll", $fabEl[0].f7FabMorphResizeHandler);
    }
