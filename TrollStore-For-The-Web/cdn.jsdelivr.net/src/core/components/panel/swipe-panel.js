...
function swipePanel(panel) {
  ...
  function handleTouchStart(e) {
    ...
    if (
      !app.panel.allowOpen ||
      (!params.swipe && !params.swipeOnlyClose) ||
      isTouched
    )
      return;
    if (
      $(".modal-in:not(.toast):not(.notification), .photo-browser-in").length >
      0
    )
      return;
    ...
    const otherPanelOpened =
      otherPanel.opened &&
      otherPanel.$el &&
      !otherPanel.$el.hasClass("panel-in-breakpoint");
  ...
  function handleTouchMove(e) {
    ...
    if (
      e.f7PreventSwipePanel ||
      app.preventSwipePanelBySwipeBack ||
      app.preventSwipePanel
    ) {
      isTouched = false;
      return;
    }
    ...
    if (!direction) {
      ...
      if (params.swipeActiveArea > 0 && !panel.opened) {
        ...
        if (
          side === "right" &&
          touchesStart.x < app.width - params.swipeActiveArea
        ) {
          isTouched = false;
          return;
        }
    ...
    if (effect === "reveal") {
      ...
      if (!params.swipeNoFollow) {
        ...
        if (effect === "push") {
          const viewTranslate =
            side === "left" ? translate + panelWidth : translate - panelWidth;
          $viewEl
            .transform(`translate3d(${viewTranslate}px,0,0)`)
            .transition(0);
          if ($backdropEl) {
            $backdropEl
              .transform(`translate3d(${viewTranslate}px,0,0)`)
    ...
    if (params.swipeNoFollow) {
      const stateChanged =
        (panel.opened && noFollowProgress === 0) ||
        (!panel.opened && noFollowProgress === 1);
      if (stateChanged) {
        isInterrupted = true;
  ...
  function handleTouchEnd(e) {
    ...
    const startTranslate =
      effect === "floating" ? (side === "left" ? 8 : -8) : 0;
    const edge =
      (translate === startTranslate || Math.abs(translate) === panelWidth) &&
      !params.swipeNoFollow;
