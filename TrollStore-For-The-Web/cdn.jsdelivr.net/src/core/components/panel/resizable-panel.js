...
function resizablePanel(panel) {
  ...
  function handleTouchMove(e) {
    ...
    if (!isMoved) {
      ...
      if (isPushingPanel || visibleByBreakpoint) {
        ...
        if (panel.$containerEl && panel.$containerEl.hasClass("page")) {
          $viewEl.add(
            panel.$containerEl.children(".page-content, .tabs, .fab")
          );
        }
      }
      if (isPushingPanel && !visibleByBreakpoint) {
    ...
    let newPanelWidth =
      side === "left" ? panelWidth + touchesDiff : panelWidth - touchesDiff;
    ...
    if (isPushingPanel && !visibleByBreakpoint) {
      if ($viewEl) {
        $viewEl.transform(
          `translate3d(${
            side === "left" ? newPanelWidth : -newPanelWidth
          }px, 0, 0)`
        );
      }
      if ($backdropEl) {
  ...
  function handleResize() {
    ...
    if (
      panelMinWidth &&
      !Number.isNaN(panelMinWidth) &&
      panel.resizableWidth < panelMinWidth
    ) {
      panel.resizableWidth = Math.max(panel.resizableWidth, panelMinWidth);
    }
    if (
      panelMaxWidth &&
      !Number.isNaN(panelMaxWidth) &&
      panel.resizableWidth > panelMaxWidth
    ) {
      panel.resizableWidth = Math.min(panel.resizableWidth, panelMaxWidth);
    }
    panel.resizableWidth = Math.min(
      Math.max(panel.resizableWidth, 0),
      app.width
    );
