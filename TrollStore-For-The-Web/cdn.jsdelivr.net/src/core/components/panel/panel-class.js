...
class Panel extends Framework7Class {
  constructor(app, params = {}) {
    ...
    panel.$containerEl = panel.params.containerEl
      ? $(panel.params.containerEl).eq(0)
      : app.$el;
  ...
  setStateClasses(state) {
    ...
    if (state === "closing") {
      ...
      $targetEl.removeClass(
        `with-panel with-panel-${panel.side}-${panel.effect}`
      );
    }
  ...
  setVisibleBreakpoint(emitEvents = true) {
    ...
    if (!panel.visibleBreakpointResizeHandler) {
      panel.visibleBreakpointResizeHandler =
        function visibleBreakpointResizeHandler() {
          panel.setVisibleBreakpoint();
        };
      app.on("resize", panel.visibleBreakpointResizeHandler);
    }
    const { side, $el, $containerEl, params, visibleBreakpointDisabled } =
      panel;
  ...
  onClosed() {
    ...
    if (panel.$backdropEl) {
      ...
      const shouldHideBackdrop =
        !otherPanel || (otherPanel && !otherPanel.$backdropEl);
  ...
  insertToRoot() {
    ...
    if (
      !$panelParentEl.is($containerEl) ||
      $el.prevAll(".views, .view").length
    ) {
      const $insertBeforeEl = $containerEl
        .children(".panel, .views, .view")
        .eq(0);
  ...
  open(animate = true) {
    ...
    if (
      opened ||
      $el.hasClass("panel-in-breakpoint") ||
  ...
  destroy() {
    ...
    if (
      panel.$el.hasClass("panel-in-breakpoint") ||
      panel.$el.hasClass("panel-in-collapsed")
    ) {
      const $viewEl = $(panel.getViewEl());
      if ($containerEl && $containerEl.hasClass("page")) {
        $viewEl.add($containerEl.children(".page-content, .tabs"));
