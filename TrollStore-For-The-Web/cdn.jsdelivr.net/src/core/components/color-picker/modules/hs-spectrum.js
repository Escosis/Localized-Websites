...
export default {
  ...
  init(self) {
    ...
    function handleTouchStart(e) {
      ...
      specterHandleIsTouched =
        $targetEl.closest(".color-picker-hs-spectrum-handle").length > 0;
      if (!specterHandleIsTouched) {
        specterIsTouched =
          $targetEl.closest(".color-picker-hs-spectrum").length > 0;
      }
      if (specterIsTouched) {
        specterRect = $el
          .find(".color-picker-hs-spectrum")[0]
  ...
  update(self) {
    ...
    const specterWidth = self.$el.find(".color-picker-hs-spectrum")[0]
      .offsetWidth;
    const specterHeight = self.$el.find(".color-picker-hs-spectrum")[0]
      .offsetHeight;
    ...
    self.$el
      ...
      .transform(
        `translate(${specterWidth * (hsb[0] / 360)}px, ${
          specterHeight * (1 - hsb[1])
        }px)`
      );
  },
  destroy(self) {
