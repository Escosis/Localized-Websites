...
export default {
  ...
  init(self) {
    ...
    function handleTouchStart(e) {
      ...
      wheelHandleIsTouched =
        $targetEl.closest(".color-picker-wheel-handle").length > 0;
      wheelIsTouched = $targetEl.closest("circle").length > 0;
      specterHandleIsTouched =
        $targetEl.closest(".color-picker-sb-spectrum-handle").length > 0;
      if (!specterHandleIsTouched) {
        specterIsTouched =
          $targetEl.closest(".color-picker-sb-spectrum").length > 0;
      }
      if (wheelIsTouched) {
      ...
      if (specterIsTouched) {
        specterRect = $el
          .find(".color-picker-sb-spectrum")[0]
  ...
  update(self) {
    ...
    const specterWidth = self.$el.find(".color-picker-sb-spectrum")[0]
      .offsetWidth;
    const specterHeight = self.$el.find(".color-picker-sb-spectrum")[0]
      .offsetHeight;
    ...
    const tX =
      wheelHalfSize -
      Math.sin(angleRad) * (wheelHalfSize - handleHalfSize) -
      handleHalfSize;
    const tY =
      wheelHalfSize -
      Math.cos(angleRad) * (wheelHalfSize - handleHalfSize) -
      handleHalfSize;
    self.$el
    ...
    self.$el
      ...
      .transform(
        `translate(${specterWidth * hsb[1]}px, ${
          specterHeight * (1 - hsb[2])
        }px)`
      );
  },
  destroy(self) {
