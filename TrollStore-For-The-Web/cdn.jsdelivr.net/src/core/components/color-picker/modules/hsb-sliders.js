...
export default {
  ...
  init(self) {
    ...
    self.$el.on(
      "change",
      ".color-picker-module-hsb-sliders input",
      handleInputChange
    );
  ...
  update(self) {
    ...
    self.saturationRangeSlider.$el
      ...
      .css(
        "background-image",
        `linear-gradient(${app.rtl ? "to left" : "to right"}, hsl(${
          hslLeft[0]
        }, ${hslLeft[1] * 100}%, ${hslLeft[2] * 100}%), hsl(${hslRight[0]}, ${
          hslRight[1] * 100
        }%, ${hslRight[2] * 100}%))`
      );
    ...
    if (sliderValue && sliderValueEditable) {
      ...
      self.$el
        .find("input.color-picker-value-saturation")
        .val(`${(hsb[1] * 1000) / 10}`);
      self.$el
        .find("input.color-picker-value-brightness")
        .val(`${(hsb[2] * 1000) / 10}`);
    } else if (sliderValue) {
      self.$el.find("span.color-picker-value-hue").text(`${hue}`);
      self.$el
        .find("span.color-picker-value-saturation")
        .text(`${(hsb[1] * 1000) / 10}`);
      self.$el
        .find("span.color-picker-value-brightness")
        .text(`${(hsb[2] * 1000) / 10}`);
    }
  },
  destroy(self) {
