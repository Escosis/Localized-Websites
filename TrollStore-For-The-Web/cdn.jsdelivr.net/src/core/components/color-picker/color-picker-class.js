...
class ColorPicker extends Framework7Class {
  constructor(app, params = {}) {
    ...
    function onHtmlClick(e) {
      ...
      if (
        $clickTargetEl.closest(".color-picker-popup, .color-picker-popover")
          .length
      )
        return;
  ...
  get view() {
    ...
    if (params.view) {
      ...
      if (!view && $targetEl) {
        view =
          $targetEl.parents(".view").length &&
          $targetEl.parents(".view")[0].f7View;
      }
    }
  ...
  renderNavbar() {
    ...
    const { openIn, navbarTitleText, navbarBackLinkText, navbarCloseText } =
      self.params;
    return (
      <div class="navbar">
  ...
  onOpen() {
    const self = this;
    const { initialized, $el, app, $inputEl, inline, value, params, modules } =
      self;
    self.closing = false;
    self.opened = true;
    self.opening = true;
  ...
  close() {
    ...
    if (
      (self.params.routableModals && self.view) ||
      self.params.openIn === "page"
    ) {
