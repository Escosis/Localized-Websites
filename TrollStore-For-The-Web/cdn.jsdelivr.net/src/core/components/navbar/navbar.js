...
const Navbar = {
  size(el) {
    ...
    const needLeftTitle =
      app.theme === "ios" && !app.params.navbar[`${app.theme}CenterTitle`];
    ...
    const left = app.rtl
      ? $innerEl.children(".right")
      : $innerEl.children(".left");
    const right = app.rtl
      ? $innerEl.children(".left")
      : $innerEl.children(".right");
    ...
    if (dynamicNavbar && app.theme === "ios") {
      if (title.hasClass("sliding") || (title.length > 0 && sliding)) {
        ...
        const titleRightOffset =
          (navbarInnerWidth - currLeft - diff - titleWidth) * inverter;
      ...
      if (!noLeft && (left.hasClass("sliding") || sliding)) {
        if (app.rtl) {
          left[0].f7NavbarLeftOffset =
            (-(navbarInnerWidth - left[0].offsetWidth) / 2) * inverter;
          left[0].f7NavbarRightOffset = leftWidth * inverter;
        } else {
          left[0].f7NavbarLeftOffset = -leftWidth;
          left[0].f7NavbarRightOffset =
            (navbarInnerWidth - left[0].offsetWidth) / 2;
          if (
            router &&
            ...
            if (left.find(".back .icon ~ span").length) {
              ...
              left.find(".back .icon ~ span")[0].f7NavbarLeftOffset =
                leftOffset;
      ...
      if (!noRight && (right.hasClass("sliding") || sliding)) {
        if (app.rtl) {
          ...
          right[0].f7NavbarRightOffset =
            ((navbarInnerWidth - right[0].offsetWidth) / 2) * inverter;
        } else {
          right[0].f7NavbarLeftOffset =
            -(navbarInnerWidth - right[0].offsetWidth) / 2;
  ...
  initNavbarOnScroll(
    ...
    const snapPageScrollToLargeTitle =
      app.params.navbar.snapPageScrollToLargeTitle;
    const snapPageScrollToTransparentNavbar =
      app.params.navbar.snapPageScrollToTransparentNavbar;
    ...
    function snapLargeNavbar() {
      const inSearchbarExpanded = $navbarEl.hasClass(
    ...
    function snapTransparentNavbar() {
      ...
      if (!scrollContent || currentScrollTop < 0) return;
      if (
        currentScrollTop >= navbarOffsetHeight / 2 &&
        currentScrollTop < navbarOffsetHeight
      ) {
    ...
    function handleNavbarTransparent() {
      ...
      if (
        (notTransparent && opacity === 1) ||
        (!notTransparent && opacity === 0)
      ) {
    ...
    function handleLargeNavbarCollapse(pageContentEl) {
      ...
      collapseProgress = Math.min(
        Math.max(currentScrollTop / scrollableDistance, 0),
        1
      );
      ...
      if (
        ...
        $navbarEl
          ...
          .css(
            "transform",
            `translate3d(0px, ${
              -1 * collapseProgress * navbarTitleLargeHeight
            }px, 0)`
          );
        $navbarEl
          .find(".title-large-text")
          .css("opacity", 1 - collapseProgress * 2);
    ...
    function handleTitleHideShow() {
      ...
      if (reachEnd) {
        ...
        if (
          app.params.navbar.showOnPageScrollTop ||
          currentScrollTop <= navbarHideHeight
        ) {
          action = "show";
        } else {
          action = "hide";
    ...
    $pageEl[0].f7DetachNavbarScrollHandlers =
      function f7DetachNavbarScrollHandlers() {
...
export default {
  ...
  on: {
    ...
    pageBeforeIn(page) {
      ...
      if (
        page.$el.hasClass("no-navbar") ||
        (view.router.dynamicNavbar && !navbarEl)
      ) {
