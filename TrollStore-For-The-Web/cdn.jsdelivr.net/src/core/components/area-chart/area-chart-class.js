...
class AreaChart extends Framework7Class {
  ...
  getVisibleLabels() {
    ...
    const filtered = axisLabels.filter(
      (label, index) => index % skipStep === 0
    );
    return filtered;
  }
  ...
  toggleDataset(index) {
    ...
    if (this.$legendEl) {
      this.$legendEl
        .find(".area-chart-legend-item")
        .removeClass("area-chart-legend-item-hidden");
  ...
  formatTooltip() {
    ...
    const totalText = formatTooltipTotal
      ? formatTooltipTotal.call(self, total)
      : total;
    // prettier-ignore
  ...
  setTooltip() {
    ...
    const hasVisibleDataSets =
      datasets.filter((dataset, index) => !hiddenDatasets.includes(index))
        .length > 0;
  ...
  setCurrentIndex(index) {
    ...
    this.$svgEl
      .find(`line[data-index="${index}"]`)
      .addClass("area-chart-current-line");
    this.setTooltip();
  }

  onLegendClick(e) {
    const index = parseInt(
      $(e.target).closest(".area-chart-legend-item").attr("data-index"),
      10
    );
  ...
  onMouseMove(e) {
    ...
    const distances = self.linesOffsets.map((left) =>
      Math.abs(currentLeft - left)
    );
        ...
        {axis && (
          <div class="area-chart-axis">
            {axisLabels.map((label) => (
              <span>
                {visibleLegends.includes(label) && (
                  <span>{self.formatAxisLabel(label)}</span>
                )}
              </span>
            ))}
          </div>
        ...
        {legend && (
          <div class="area-chart-legend">
            {datasets.map((dataset, index) => (
              <LegendItemTag
                data-index={index}
                class={`area-chart-legend-item ${
                  toggleDatasets ? "area-chart-legend-button" : ""
                }`}
