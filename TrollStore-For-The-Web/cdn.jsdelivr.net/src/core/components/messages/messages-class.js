...
class Messages extends Framework7Class {
    ...
    return (
      <div
        class={`message message-${message.type} ${
          message.isTyping ? "message-typing" : ""
        } ${message.cssClass || ""}`}
        {...message.attrs}
      >
        {message.avatar && (
  ...
  renderMessages(
    ...
    const m = this;
    const html = messagesToRender
      .map((message) => m.renderMessage(message))
      .join("");
    m.$el[method](html);
  }
  ...
  isSameNameMessage(...args) {
    ...
    if (m.params.sameNameMessageRule)
      return m.params.sameNameMessageRule(...args);
    return false;
  }
  ...
  isSameHeaderMessage(...args) {
    ...
    if (m.params.sameHeaderMessageRule)
      return m.params.sameHeaderMessageRule(...args);
    return false;
  }
  ...
  isSameFooterMessage(...args) {
    ...
    if (m.params.sameFooterMessageRule)
      return m.params.sameFooterMessageRule(...args);
    return false;
  }
  ...
  isSameAvatarMessage(...args) {
    ...
    if (m.params.sameAvatarMessageRule)
      return m.params.sameAvatarMessageRule(...args);
    return false;
  }
  ...
  isCustomClassMessage(...args) {
    ...
    if (m.params.customClassMessageRule)
      return m.params.customClassMessageRule(...args);
    return undefined;
  }
  ...
  layout() {
    ...
    m.$el.find(".message, .messages-title").each((messageEl, index) => {
      ...
      let customMessageClasses = m.isCustomClassMessage(
        message,
        previousMessage,
        nextMessage
      );
  ...
  removeMessages(messagesToRemove, layout = true) {
    ...
    if (Array.isArray(messagesToRemove)) {
      ...
      messagesToRemove.forEach((messageToRemoveIndex) => {
        messagesToRemoveEls.push(
          m.$el.find(".message, .messages-title").eq(messageToRemoveIndex)
        );
      });
    ...
    messagesToAdd.forEach((messageToAdd) => {
      if (typingMessage) {
        if (method === "append") {
          ...
          m.messages.splice(
            m.messages.indexOf(typingMessage) + 1,
            0,
            messageToAdd
          );
        }
  ...
  hideTyping() {
    ...
    if (typeof typingMessageIndex !== "undefined") {
      if (
        m.$el.find(".message").eq(typingMessageIndex).hasClass("message-typing")
      ) {
        typingFound = true;
