...
const textEditorButtonsMap = {
  ...
  h1: [
    '<i class="icon">H<sub>1</sub></i>',
    '<i class="icon">H<sub>1</sub></i>',
    "formatBlock.H1",
  ],
  h2: [
    '<i class="icon">H<sub>2</sub></i>',
    '<i class="icon">H<sub>2</sub></i>',
    "formatBlock.H2",
  ],
  h3: [
    '<i class="icon">H<sub>3</sub></i>',
    '<i class="icon">H<sub>3</sub></i>',
    "formatBlock.H3",
  ],
  ...
  subscript: [
    ...
    '<i class="icon">A<sub>1</sub></i>',
    "subscript",
  ],
  superscript: [
    "textformat_superscript",
    '<i class="icon">A<sup>1</sup></i>',
    "superscript",
  ],
...
class TextEditor extends Framework7Class {
  constructor(app, params) {
    ...
    self.attachEvents = function attachEvents() {
      if (self.params.mode === "toolbar") {
        self.$el
          .find(".text-editor-toolbar")
          .on("click", "button", self.onButtonClick);
      }
      if (self.params.mode === "keyboard-toolbar") {
        self.$keyboardToolbarEl.on("click", "button", self.onButtonClick);
        self.$el
          .parents(".page")
          .on("page:beforeout", self.closeKeyboardToolbar);
      }
    ...
    self.detachEvents = function detachEvents() {
      if (self.params.mode === "toolbar") {
        self.$el
          .find(".text-editor-toolbar")
          .off("click", "button", self.onButtonClick);
      }
      if (self.params.mode === "keyboard-toolbar") {
        self.$keyboardToolbarEl.off("click", "button", self.onButtonClick);
        self.$el
          .parents(".page")
          .off("page:beforeout", self.closeKeyboardToolbar);
      }
      if (self.params.mode === "popover" && self.popover) {
  ...
  createLink() {
    ...
    if (
      ...
      while (anchorNode) {
        ...
        if (
          ...
          anchorNode === currentSelection.focusNode
        ) {
          anchorNode = null;
        }
        if (anchorNode) {
    ...
    const dialog = self.app.dialog.prompt(
      "",
      self.params.linkUrlText,
      (link) => {
    ...
    const dialog = self.app.dialog.prompt(
      ...
      (imageUrl) => {
        if (imageUrl && imageUrl.trim().length) {
          ...
          self.emit(
            "local:insertImage textEditorInsertImage",
            self,
            imageUrl.trim()
          );
        }
  ...
  insertPlaceholder() {
    ...
    self.$contentEl.append(
      `<div class="text-editor-placeholder">${self.params.placeholder}</div>`
    );
  }
  ...
  onPaste(e) {
    ...
    const document = getDocument();
    if (
      self.params.clearFormattingOnPaste &&
      e.clipboardData &&
  ...
  onButtonClick(e) {
    ...
    const buttonData =
      self.params.customButtons && self.params.customButtons[button];
    ...
    if (command.indexOf("formatBlock") === 0) {
      ...
      if (
        $anchorNode.parents(tagName.toLowerCase()).length ||
        $anchorNode.is(tagName)
      ) {
  ...
  renderButtons() {
    ...
    function renderButton(button) {
      ...
      const iconContent =
        textEditorButtonsMap[button][self.app.theme === "md" ? 1 : 0];
      return `<button type="button" class="text-editor-button" data-button="${button}">${
        iconContent.indexOf("<") >= 0
          ? iconContent
          : `<i class="${iconClass}">${iconContent}</i>`
      }</button>`.trim();
    }
    self.params.buttons.forEach((button, buttonIndex) => {
      if (Array.isArray(button)) {
        ...
        if (
          buttonIndex < self.params.buttons.length - 1 &&
          self.params.dividers
        ) {
  ...
  createToolbar() {
    const self = this;
    self.$el.prepend(
      `<div class="text-editor-toolbar">${self.renderButtons()}</div>`
    );
  }
