...
class Messagebar extends Framework7Class {
  constructor(app, params = {}) {
    ...
    extend(messagebar, {
      ...
      attachmentsVisible: $attachmentsEl.hasClass(
      ...
      */
      ...
      if (
        ...
        const scrollOnBottom =
          pageScrollTop === pageScrollHeight - pageOffsetHeight;
        if (!maxHeight) {
          maxHeight =
        ...
        if (scrollOnBottom) {
          $pageContentEl.scrollTop(
            $pageContentEl[0].scrollHeight - pageOffsetHeight
          );
        }
        $el.trigger("messagebar:resizepage");
  ...
  attachmentsCreate(innerHTML = "") {
    const messagebar = this;
    const $attachmentsEl = $(
      `<div class="messagebar-attachments">${innerHTML}</div>`
    );
  ...
  renderAttachments() {
    ...
    if (messagebar.params.renderAttachments) {
      html = messagebar.params.renderAttachments.call(
        messagebar,
