...
export default function pickerColumn(colEl, updateItems) {
  ...
  col.calcSize = function calcColSize() {
    ...
    const hadPadding = col.el.style.getPropertyValue(
      "--f7-picker-scroll-padding"
    );
    col.el.style.setProperty(
      "--f7-picker-scroll-padding",
      `${(colHeight - itemHeight) / 2}px`
    );
    if (!hadPadding) {
  ...
  col.updateItems = function updateColItems(
    activeIndex,
    scrollTop,
    valueCallbacks
  ) {
    ...
    if (picker.params.rotateEffect) {
      col.items.each((itemEl) => {
        ...
        const itemOffset =
          itemEl.offsetTop - (colHeight - itemHeight) / 2 - scrollTop;
        ...
        $itemEl
          ...
          .transform(
            `translate3d(0, ${
              -percentage * itemHeight
            }px, -100px) rotateX(${angle}deg)`
          );
      });
    }
