...
class Picker extends Framework7Class {
      ...
      for (let i = 0; i < noDividerColumns.length; i += 1) {
        ...
        if (
          ...
          newDisplayValue.push(
            column.displayValues[column.values.indexOf(newValue[i])]
          );
        } else {
  ...
  renderColumn(col, onlyItems) {
    const colClasses = `picker-column ${
      col.textAlign ? `picker-column-${col.textAlign}` : ""
    } ${col.cssClass || ""}`;
    let columnHtml;
    let columnItemsHtml;
  ...
  renderInline() {
    ...
    const inlineHtml = (
      <div
        class={`picker picker-inline ${rotateEffect ? "picker-3d" : ""} ${
          cssClass || ""
        }`}
      >
  ...
  renderSheet() {
    ...
    const sheetHtml = (
      <div
        class={`sheet-modal picker picker-sheet ${
          rotateEffect ? "picker-3d" : ""
        } ${cssClass || ""}`}
      >
        {toolbar && picker.renderToolbar()}
    ...
    const popoverHtml = (
      <div class="popover picker-popover">
        <div class="popover-inner">
          <div
            class={`picker ${rotateEffect ? "picker-3d" : ""} ${
              cssClass || ""
            }`}
          >
            {toolbar && picker.renderToolbar()}
  ...
  open() {
    ...
    const modalParams = {
      ...
      backdrop:
        typeof params.backdrop !== "undefined" ? params.backdrop : isPopover,
      on: {
        open() {
          const modal = this;
          picker.modal = modal;
