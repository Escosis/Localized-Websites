...
const CardExpandable = {
  open(cardEl = ".card-expandable", animate = true) {
    ...
    const $cardSizeEl = $(document.createElement("div")).addClass(
    ...
    if (
      $navbarEl &&
      !cardParams.hideStatusbarOnOpen &&
    ...
    if (hasTransform) {
      ...
      if (transformValues && transformValues.length > 1) {
        ...
        cardTopOffset =
          offset.top - pageOffset.top - (cardHeight * (1 - scale)) / 2;
        if (app.rtl) cardLeftOffset -= $cardEl[0].scrollLeft;
      } else {
        cardLeftOffset = $cardEl[0].offsetLeft;
        cardTopOffset =
          $cardEl[0].offsetTop -
          ($pageContentEl.length ? $pageContentEl[0].scrollTop : 0);
      }
    } else {
    ...
    function onTouchMove(e) {
      ...
      if (progress > (isV ? 1.3 : 1.1)) {
        ...
        $cardEl.transform(
          `translate3d(${
            app.rtl ? -translateX : translateX
          }px, ${translateY}px, 0) scale(${scaleX * (1 - progress * 0.2)}, ${
    ...
    pageBeforeIn(page) {
      ...
      if (
        ...
        app.navbar.hide(
          page.navbarEl,
          true,
          app.params.card.hideStatusbarOnOpen,
          true
        );
      }
  ...
  clicks: {
    ...
    ".card-backdrop-in": function onBackdropClick() {
      ...
      if ($openedCardEl.attr("data-close-by-backdrop-click") === "true") {
        ...
        $openedCardEl.attr("data-close-by-backdrop-click") === "false"
      ) {
        needToClose = false;
      }
