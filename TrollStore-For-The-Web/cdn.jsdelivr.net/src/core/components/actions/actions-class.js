...
class Actions extends Modal {
  constructor(app, params) {
    ...
    actions.open = function open(animate) {
      ...
      const { targetEl, targetX, targetY, targetWidth, targetHeight } =
        actions.params;
      if (
        ...
        popover = app.popover.create({
          ...
          on: {
            open() {
              ...
              actions.$el.trigger(
                `modal:open ${actions.type.toLowerCase()}:open`
              );
              actions.emit(
                `local::open modalOpen ${actions.type}Open`,
                actions
              );
            },
            opened() {
              ...
              actions.$el.trigger(
                `modal:opened ${actions.type.toLowerCase()}:opened`
              );
              actions.emit(
                `local::opened modalOpened ${actions.type}Opened`,
                actions
              );
            },
            close() {
              ...
              actions.$el.trigger(
                `modal:close ${actions.type.toLowerCase()}:close`
              );
              actions.emit(
                `local::close modalClose ${actions.type}Close`,
                actions
              );
            },
            closed() {
              ...
              actions.$el.trigger(
                `modal:closed ${actions.type.toLowerCase()}:closed`
              );
              actions.emit(
                `local::closed modalClosed ${actions.type}Closed`,
                actions
              );
            },
          },
  ...
  render() {
    ...
    return (
      <div
        class={`actions-modal${actions.params.grid ? " actions-grid" : ""} ${
          cssClass || ""
        }`}
      >
        {groups.map((group) => (
          ...
          {groups.map((group) => (
            <div class="list">
              <ul>
                {group.map((button) => {
                  const itemClasses = [];
                  const { color, bg, strong, disabled, label, text, icon } =
                    button;
