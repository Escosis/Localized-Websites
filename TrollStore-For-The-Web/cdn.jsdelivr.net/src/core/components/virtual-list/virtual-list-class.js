...
class VirtualList extends Framework7Class {
      ...
      $tabEl = vl.$el
        ...
        .filter((tabEl) => {
          return (
            $(tabEl)
              .parent(".tabs")
              .parent(".tabs-animated-wrap, swiper-container.tabs").length === 0
          );
        })
        .eq(0);
  ...
  setListSize(autoHeightRerender) {
    ...
    if (vl.dynamicHeight) {
      ...
      vl.$itemsWrapEl.find(`[data-virtual-list-index]`).forEach((el) => {
        renderedItems[
          parseInt(el.getAttribute("data-virtual-list-index"), 10)
        ] = el;
      });
      for (let i = 0; i < items.length; i += 1) {
      ...
      vl.listHeight =
        Math.ceil(items.length / vl.params.cols) * vl.params.height;
  ...
  render(force, forceScrollTop) {
    ...
    let scrollTop = -(
      vl.$el[0].getBoundingClientRect().top -
      vl.$scrollableParentEl[0].getBoundingClientRect().top
    );
    ...
    if (vl.dynamicHeight || vl.autoHeight) {
      ...
      for (let j = 0; j < vl.heights.length; j += 1) {
        ...
        if (typeof toIndex === "undefined") {
          if (
            itemTop + itemHeight >=
              scrollTop +
                vl.pageHeight * 2 * vl.params.dynamicHeightBufferSize ||
      ...
      fromIndex =
        (parseInt(scrollTop / vl.params.height, 10) - vl.rowsBefore) *
        vl.params.cols;
      if (fromIndex < 0) {
        fromIndex = 0;
      }
      toIndex = Math.min(
        fromIndex + vl.rowsToRender * vl.params.cols,
        items.length
      );
    }
    ...
    if (!vl.updatableScroll) {
      if (vl.dynamicHeight || vl.autoHeight) {
        ...
        vl.itemsWrapEl.style.height = `${
          (i * vl.params.height) / vl.params.cols
        }px`;
      }
    }
  ...
  scrollToItem(index) {
    ...
    vl.render(
      ...
      listTop +
        itemTop -
        parseInt(vl.$scrollableParentEl.css("padding-top"), 10)
    );
    return true;
  }
