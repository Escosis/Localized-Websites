...
const Swipeout = {
    ...
    function handleTouchMove(e) {
      ...
      if ($actionsRight.length > 0) {
        ...
        if (buttonTranslate < -actionsRightWidth) {
          ...
          buttonTranslate =
            -actionsRightWidth - (-buttonTranslate - actionsRightWidth) ** 0.8;
          translate = buttonTranslate;
          if (
            $overswipeRightButton.length > 0 &&
            ratio > app.params.swipeout.overswipeRatio
          ) {
            overswipeRight = true;
          }
        }
      ...
      if ($actionsLeft.length > 0) {
        ...
        if (buttonTranslate > actionsLeftWidth) {
          const ratio = buttonTranslate / actionsRightWidth;
          buttonTranslate =
            actionsLeftWidth + (buttonTranslate - actionsLeftWidth) ** 0.8;
          translate = buttonTranslate;
          if (
        ...
        $leftButtons.each((buttonEl, index) => {
          ...
          $buttonEl.transform(
            `translate3d(${
              buttonTranslate + buttonOffset * (1 - Math.min(progress, 1))
            }px,0,0)`
          );
        });
      }
    ...
    function handleTouchEnd() {
      ...
      $swipeoutContent.transitionEnd(() => {
        if ((opened && action === "open") || (!opened && action === "close"))
          return;
        $swipeoutEl.trigger(
          action === "open" ? "swipeout:opened" : "swipeout:closed"
        );
        app.emit(
          action === "open" ? "swipeoutOpened" : "swipeoutClosed",
          $swipeoutEl[0]
        );
        $swipeoutEl.removeClass("swipeout-transitioning");
      ...
      if (Swipeout.el) {
        ...
        if (
          !(
            ...
            $targetEl.parents(".actions-modal.modal-in, .dialog.modal-in")
              .length > 0
          )
        ) {
  ...
  open(...args) {
    ...
    const translate =
      side === "right" ? -swipeoutActionsWidth : swipeoutActionsWidth;
    if ($buttons.length > 1) {
