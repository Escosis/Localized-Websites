...
const Tab = {
  show(...args) {
    ...
    if (
      !animatedInit &&
      ($newTabEl.length === 0 || $newTabEl.hasClass("tab-active"))
    ) {
      return {
        $newTabEl,
        newTabEl: $newTabEl[0],
    ...
    if ($tabsEl.parent().hasClass("tabs-animated-wrap")) {
      ...
      const transitionDuration = parseFloat(
        $tabsEl.css("transition-duration").replace(",", ".")
      );
      ...
      const tabsTranslate =
        (app.rtl ? $newTabEl.index() : -$newTabEl.index()) * 100;
    ...
    let swiper;
    if (
      $tabsEl[0].nodeName.toLowerCase() === "swiper-container" &&
      app.swiper
    ) {
    ...
    if (
      !animatedInit &&
      (!swiper || (swiper && !swiper.animating) || (swiper && tabRoute))
    ) {
    ...
    if (!$tabLinkEl) {
      ...
      if (
        tabRoute &&
        (!$tabLinkEl || ($tabLinkEl && $tabLinkEl.length === 0))
      ) {
      ...
      if ($tabLinkEl.length > 1 && $newTabEl.parents(".page").length) {
        ...
        $tabLinkEl = $tabLinkEl.filter((tabLinkElement) => {
          return (
            $(tabLinkElement).parents(".page")[0] ===
            $newTabEl.parents(".page")[0]
          );
        });
        if (app.theme === "ios" && $tabLinkEl.length === 0 && tabRoute) {
          ...
          $tabLinkEl = $navbarEl.find(
            `[data-route-tab-id="${tabRoute.route.tab.id}"]`
          );
          if ($tabLinkEl.length === 0) {
      ...
      if ($oldTabEl && $oldTabEl.length > 0) {
        ...
        if (!$oldTabLinkEl || ($oldTabLinkEl && $oldTabLinkEl.length === 0)) {
          $("[data-tab]").each((tabLinkElement) => {
            if ($oldTabEl.is($(tabLinkElement).attr("data-tab")))
              $oldTabLinkEl = $(tabLinkElement);
          });
        }
        if (!$oldTabLinkEl || ($oldTabLinkEl && $oldTabLinkEl.length === 0)) {
