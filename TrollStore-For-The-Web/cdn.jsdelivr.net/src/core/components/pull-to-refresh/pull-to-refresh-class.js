  ...
  constructor(app, el) {
    ...
    ptr.done = function done() {
      ...
      const onTranstionEnd = (e) => {
        ...
        $el.removeClass(
          "ptr-transitioning ptr-pull-up ptr-pull-down ptr-closing"
        );
        $el.trigger("ptr:done");
      ...
      $el
        .removeClass("ptr-refreshing")
        .addClass("ptr-transitioning ptr-closing");
      return ptr;
    };
    ...
    function setPreloaderProgress(progress = 0) {
      ...
      $bars.forEach((barEl, barIndex) => {
        const barProgress =
          (progress - barIndex * perBarProgress) / perBarProgress;
    ...
    function handleTouchStart(e) {
      ...
      if (
        $(e.target).closest(
          ".sortable-handler, .ptr-ignore, .card-expandable.card-opened"
        ).length
      )
        return;
    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
              ...
              ((!ptr.bottom && ptrScrollableEl.scrollTop > 0) ||
                (ptr.bottom &&
                  ptrScrollableEl.scrollTop <
                    ptrScrollableEl.scrollHeight -
                      ptrScrollableEl.offsetHeight))
            ) {
        ...
        if (dynamicTriggerDistance) {
          ...
          if (triggerDistance.indexOf("%") >= 0)
            triggerDistance =
              (scrollHeight * parseInt(triggerDistance, 10)) / 100;
        }
      ...
      const ptrStarted = ptr.bottom
        ? (touchesDiff < 0 && scrollTop >= maxScrollTop) ||
      ...
      if (ptrStarted) {
        ...
        if (useTranslate || forceUseTranslate) {
          ...
          translate =
            (ptr.bottom
              ? -1 * Math.abs(touchesDiff) ** 0.85
              : touchesDiff ** 0.85) + startTranslate;
          if (isMaterial) {
            $preloaderEl
              .transform(`translate3d(0,${translate}px,0)`)
              .find(".ptr-arrow")
              .transform(
                `rotate(${180 * (Math.abs(touchesDiff) / 66) + 100}deg)`
              );
          } else {
    ...
    function handleMouseWheel(e) {
      ...
      if (ptrStarted) {
        ...
        if (Math.abs(translate) > triggerDistance) {
          translate =
            triggerDistance + (Math.abs(translate) - triggerDistance) ** 0.7;
