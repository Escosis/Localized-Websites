...
function initSwiper(swiperEl) {
  ...
  if ($swiperEl.hasClass("tabs")) {
    initialSlide = $swiperEl
      .children("swiper-slide")
      .indexOf($swiperEl.children(".tab-active")[0]);
    isTabs = true;
  ...
  if (
    typeof params.initialSlide === "undefined" &&
    typeof initialSlide !== "undefined"
  ) {
    params.initialSlide = initialSlide;
  }

  const swiper = isElement
    ? $swiperEl[0].swiper
  ...
  if (isTabs) {
    swiper.on("slideChange", () => {
      if (isRoutableTabs) {
        ...
        const tabRouteUrl = router.findTabRouteUrl(
          $(swiper.slides).eq(swiper.activeIndex)[0]
        );
        if (tabRouteUrl) {
          setTimeout(() => {
