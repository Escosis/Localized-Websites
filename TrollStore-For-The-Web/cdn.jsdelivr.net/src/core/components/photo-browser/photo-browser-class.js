...
class PhotoBrowser extends Framework7Class {
  ...
  onSlideChange(swiper) {
    ...
    let total = pb.params.virtualSlides
      ? pb.params.photos.length
    ...
    const $activeSlideEl = pb.params.virtualSlides
      ? $(swiper.wrapperEl).find(
          `.swiper-slide[data-swiper-slide-index="${swiper.activeIndex}"]`
        )
      : $(swiper.slides).eq(swiper.activeIndex);
    const $previousSlideEl = pb.params.virtualSlides
      ? $(swiper.wrapperEl).find(
          `.swiper-slide[data-swiper-slide-index="${swiper.previousIndex}"]`
        )
  ...
  onTouchMove(e) {
    ...
    if (!swipeToClose.started) {
      ...
      if (pb.params.virtualSlides) {
        swipeToClose.activeSlide = $(pb.swiper.wrapperEl).children(
          ".swiper-slide-active"
        );
      } else {
        swipeToClose.activeSlide = $(pb.swiper.slides).eq(
          pb.swiper.activeIndex
        );
      }
      swipeToClose.timeStart = now();
    }
    e.preventDefault();
    swipeToClose.current =
      e.type === "touchmove" ? e.targetTouches[0].pageY : e.pageY;
  ...
  renderNavbar() {
    ...
    const pageBackLinkText =
      pb.app.theme === "ios" && pb.params.pageBackLinkText
    ...
    return (
      ...
      >
        ...
        <div class="navbar-inner navbar-inner-centered-title sliding">
          ...
          {isPopup &&
            (pb.params.popupCloseLinkText || pb.params.popupCloseLinkIcon) && (
              <div class="right">
      ...
      <div class="toolbar toolbar-bottom photo-browser-thumbs">
        <div class="swiper">
          <div class="swiper-wrapper">
            {pb.params.thumbs.map((thumb, index) =>
              pb.renderThumb(thumb, index)
            )}
          </div>
        </div>
      </div>
  ...
  render() {
    ...
    return (
      <div class={`photo-browser photo-browser-${pb.params.theme}`}>
        <div class="view">
          <div
            class={`page photo-browser-page photo-browser-page-${
              pb.params.theme
            } no-toolbar ${!pb.params.navbar ? "no-navbar" : ""}`}
