  ...
  init() {
    ...
    function handleTouchMove(e) {
      ...
      const translateScrollOffset =
        $pageContentEl[0].scrollTop - initialScrollTop;
      const translate = Math.min(
        Math.max(touchesDiff + translateScrollOffset, -minTop),
        maxTop
      );
      ...
      if (allowScroll) {
        ...
        if (sortingElOffsetTop + touchesDiff < pageOffset + scrollAddition) {
          // To Top
          scrollDiff =
            sortingElOffsetTop + touchesDiff - pageOffset - scrollAddition;
        }
        if (scrollDiff) {
    ...
    function handleTouchEnd() {
      ...
      if (
        typeof indexTo !== "undefined" &&
        !Number.isNaN(indexTo) &&
        indexTo !== indexFrom
      ) {
