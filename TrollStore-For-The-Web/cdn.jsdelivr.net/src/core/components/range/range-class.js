...
class Range extends Framework7Class {
  constructor(app, params) {
    ...
    "step min max value scaleSteps scaleSubSteps"
      ...
      .forEach((paramName) => {
        if (
          ...
          typeof dataset[paramName] !== "undefined"
        ) {
    ...
    if (!range.params.value) {
      ...
      if (
        typeof dataset.valueLeft !== "undefined" &&
        typeof dataset.valueRight !== "undefined"
      ) {
        range.params.value = [
          parseFloat(dataset.valueLeft),
          parseFloat(dataset.valueRight),
        ];
      }
    }
    ...
    function handleTouchStart(e) {
      ...
      if (range.vertical) {
        ...
        progress =
          (rangeOffsetLeft + range.rangeWidth - touchesStart.x) /
          range.rangeWidth;
      } else {
      ...
      if (range.dual) {
        if (
          Math.abs(range.value[0] - newValue) <
          Math.abs(range.value[1] - newValue)
        ) {
          dualValueIndex = 0;
    ...
    function handleTouchMove(e) {
      ...
      if (range.vertical) {
        ...
        progress =
          (rangeOffsetLeft + range.rangeWidth - pageX) / range.rangeWidth;
      } else {
  ...
  layout() {
    ...
    if (range.dual) {
      ...
      knobs.forEach(($knobEl, knobIndex) => {
        ...
        if (label)
          labels[knobIndex].text(
            range.formatLabel(value[knobIndex], labels[knobIndex][0])
          );
      });
    } else {
    ...
    if (
      (range.dual && value.indexOf(min) >= 0) ||
      (!range.dual && value === min)
    ) {
    ...
    if (
      (range.dual && value.indexOf(max) >= 0) ||
      (!range.dual && value === max)
    ) {
  ...
  setValue(newValue, byTouchMove) {
    ...
    if (range.dual) {
      ...
      valueChanged =
        oldValue[0] !== newValues[0] || oldValue[1] !== newValues[1];
      range.layout();
    } else {
      oldValue = range.value;
      const value = Math.max(
        Math.min(Math.round(newValue / step) * step, max),
        min
      );
      range.value = value;
      range.layout();
  ...
  renderScale() {
    ...
    Array.from({ length: range.scaleSteps + 1 }).forEach((scaleEl, index) => {
      ...
      if (
        range.scaleSubSteps &&
        range.scaleSubSteps > 1 &&
        index < range.scaleSteps
      ) {
        Array.from({ length: range.scaleSubSteps - 1 }).forEach(
          (subStepEl, subIndex) => {
            const subStep = scaleStepValue / range.scaleSubSteps;
            const scaleSubValue = scaleValue + subStep * (subIndex + 1);
            const subProgress =
              (scaleSubValue - range.min) / (range.max - range.min);
