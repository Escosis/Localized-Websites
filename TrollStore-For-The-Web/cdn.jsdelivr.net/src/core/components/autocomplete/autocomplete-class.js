...
class Autocomplete extends Framework7Class {
  constructor(app, params = {}) {
    ...
    function onInputChange() {
      ...
      ac.params.source.call(ac, query, (items) => {
        let itemsHTML = "";
        const limit = ac.params.limit
          ? Math.min(ac.params.limit, items.length)
          : items.length;
        ac.items = items;
        let regExp;
        ...
        for (let i = 0; i < limit; i += 1) {
          const itemValue =
            typeof items[i] === "object"
              ? items[i][ac.params.valueProperty]
              : items[i];
          const itemText =
            typeof items[i] === "object"
              ? items[i][ac.params.textProperty]
              : items[i];
          if (i === 0) {
          ...
          itemsHTML += ac.renderItem(
            {
              value: itemValue,
              text: ac.params.highlightMatches
                ? itemText.replace(regExp, "<b>$1</b>")
                : itemText,
            },
            i
          );
        ...
        if (
          itemsHTML === "" &&
          query === "" &&
          ac.params.dropdownPlaceholderText
        ) {
          itemsHTML += ac.renderItem({
            placeholder: true,
        ...
        if (ac.params.typeahead) {
          ...
          const previousValue =
            typeof ac.value[0] === "object"
              ? ac.value[0][ac.params.valueProperty]
              : ac.value[0];
          if (
            !previousValue ||
            firstValue.toLowerCase() !== previousValue.toLowerCase()
          ) {
            ac.value = [firstItem];
    ...
    function onPageInputChange() {
      ...
      if (isValues) {
        if (ac.inputType === "checkbox" && !inputEl.checked) {
          for (let i = 0; i < ac.value.length; i += 1) {
            aValue =
              typeof ac.value[i] === "string"
                ? ac.value[i]
                : ac.value[i][ac.params.valueProperty];
      ...
      for (let i = 0; i < ac.items.length; i += 1) {
        itemValue =
          typeof ac.items[i] === "object"
            ? ac.items[i][ac.params.valueProperty]
            : ac.items[i];
      ...
      // On Select Callback
      if (
        (ac.inputType === "radio" && inputEl.checked) ||
        ac.inputType === "checkbox"
      ) {
        ac.emit("local::change autocompleteChange", ac.value);
      }
    }
    ...
    function onKeyDown(e) {
      ...
      if (e.keyCode === 13) {
        // Enter
        const $selectedItemLabel = ac.$dropdownEl.find(
          ".autocomplete-dropdown-selected label"
        );
      ...
      e.preventDefault();
      const $selectedItem = ac.$dropdownEl.find(
      ...
      if (ac.params.updateInputValueOnSelect) {
        ac.$inputEl.val(
          typeof clickedItem === "object"
            ? clickedItem[ac.params.valueProperty]
            : clickedItem
        );
  ...
  positionDropdown() {
    ...
    const listOffsetLeft =
      $listEl.length > 0 ? listOffset.left - $pageContentEl.offset().left : 0;
    const inputOffsetLeft =
      inputOffset.left -
      ($listEl.length > 0 ? listOffset.left : 0) -
      (app.rtl ? 0 : 0);
    const inputOffsetTop =
      inputOffset.top -
      ($pageContentEl.offset().top - $pageContentEl[0].scrollTop);
    ...
    if ($listEl.length) {
      paddingValue =
        (app.rtl
          ? $listEl[0].offsetWidth - inputOffsetLeft - inputOffsetWidth
          : inputOffsetLeft) - (app.theme === "md" ? 16 : 15);
    }

    $dropdownEl.css({
      left: `${$listEl.length > 0 ? listOffsetLeft : inputOffsetLeft}px`,
      top: `${
        inputOffsetTop + $pageContentEl[0].scrollTop + inputOffsetHeight
      }px`,
      width: `${
        $listEl.length > 0 ? $listEl[0].offsetWidth : inputOffsetWidth
      }px`,
    });
    ...
    ac.params.source.call(ac, query, (items) => {
      ...
      for (let i = 0; i < limit; i += 1) {
        ...
        for (let j = 0; j < ac.value.length; j += 1) {
          const aValue =
            typeof ac.value[j] === "object"
              ? ac.value[j][ac.params.valueProperty]
              : ac.value[j];
      ...
      valuesHTML += ac.renderItem(
        {
          ...
          text:
            typeof ac.value[i] === "object"
              ? ac.value[i][ac.params.textProperty]
              : ac.value[i],
          inputType: ac.inputType,
          id: ac.id,
      ...
      <div
        class={`navbar ${
          ac.params.navbarColorTheme
            ? `color-${ac.params.navbarColorTheme}`
            : ""
        }`}
      >
    ...
    return (
      ...
      >
        ...
        <div class="page-content">
          <div
            class={`list autocomplete-list autocomplete-found autocomplete-list-${
              ac.id
            } ${
              ac.params.formColorTheme
                ? `color-${ac.params.formColorTheme}`
                : ""
            }`}
          >
            <ul></ul>
  ...
  onOpen(type, el) {
    ...
    if (ac.params.openIn === "dropdown") {
      ...
      if (
        ac.params.openIn === "page" &&
        app.theme === "ios" &&
        $searchbarEl.length === 0
      ) {
  ...
  close() {
    ...
    if (ac.params.openIn === "dropdown") {
      ...
      (ac.params.routableModals && ac.view) ||
      ac.openedIn === "page"
    ) {
