...
class ViewingConditions {
  static make(
    ...
    const xyz = whitePoint,
      rW = 0.401288 * xyz[0] + 0.650173 * xyz[1] + -0.051461 * xyz[2],
      gW = -0.250268 * xyz[0] + 1.204414 * xyz[1] + 0.045854 * xyz[2],
      bW = -0.002079 * xyz[0] + 0.048952 * xyz[1] + 0.953127 * xyz[2],
      f = 0.8 + surround / 10,
      c =
        f >= 0.9
          ? lerp(0.59, 0.69, 10 * (f - 0.9))
          : lerp(0.525, 0.59, 10 * (f - 0.8));
...
class Cam16 {
  ...
  static fromIntInViewingConditions(argb, viewingConditions) {
    const green = (65280 & argb) >> 8,
      ...
      t =
        ((5e4 / 13) *
          (0.25 * (Math.cos((huePrime * Math.PI) / 180 + 2) + 3.8)) *
  ...
  static fromUcsInViewingConditions(jstar, astar, bstar, viewingConditions) {
    const a = astar,
      b = bstar,
      m = Math.sqrt(a * a + b * b),
      c = (Math.exp(0.0228 * m) - 1) / 0.0228 / viewingConditions.fLRoot;
    let h = Math.atan2(b, a) * (180 / Math.PI);
    h < 0 && (h += 360);
    const j = jstar / (1 - 0.007 * (jstar - 100));
  ...
  static fromXyzInViewingConditions(x, y, z, viewingConditions) {
    const rC = 0.401288 * x + 0.650173 * y - 0.051461 * z,
      ...
      t =
        ((5e4 / 13) *
          ((1 / 4) * (Math.cos((huePrime * Math.PI) / 180 + 2) + 3.8)) *
          viewingConditions.nc *
          viewingConditions.ncb *
          Math.sqrt(a * a + b * b)) /
        (u + 0.305),
      ...
      s =
        ...
        Math.sqrt((alpha * viewingConditions.c) / (viewingConditions.aw + 4)),
      jstar = ((1 + 100 * 0.007) * J) / (1 + 0.007 * J),
      mstar = Math.log(1 + 0.0228 * M) / 0.0228,
      astar = mstar * Math.cos(hueRadians),
      bstar = mstar * Math.sin(hueRadians);
    return new Cam16(hue, C, J, Q, M, s, jstar, astar, bstar);
  }
...
class HctSolver {
  ...
  static bisectToSegment(y, targetHue) {
    ...
    for (let n = 0; n < 12; n++) {
      ...
      initialized
        ? (uncut || HctSolver.areInCyclicOrder(leftHue, midHue, rightHue)) &&
          ((uncut = !1),
  ...
  static findResultByJ(hueRadians, chroma, y) {
    ...
    for (let iterationRound = 0; iterationRound < 5; iterationRound++) {
      ...
      if (fnj <= 0) return 0;
      if (4 === iterationRound || Math.abs(fnj - y) < 0.002)
        return linrgb[0] > 100.01 || linrgb[1] > 100.01 || linrgb[2] > 100.01
          ? 0
  ...
  static solveToInt(hueDegrees, chroma, lstar) {
    ...
    const hueRadians =
        ((hueDegrees = sanitizeDegreesDouble(hueDegrees)) / 180) * Math.PI,
      y = yFromLstar(lstar),
...
class Blend {
  static harmonize(designColor, sourceColor) {
    const fromHct = Hct.fromInt(designColor),
      ...
      rotationDegrees = Math.min(0.5 * differenceDegrees$1, 15),
...
class Contrast {
  static ratioOfTones(toneA, toneB) {
    return (
      ...
      Contrast.ratioOfYs(yFromLstar(toneA), yFromLstar(toneB))
    );
  }
  ...
  getTone(scheme) {
    ...
    if (this.toneDeltaPair) {
      const toneDeltaPair = this.toneDeltaPair(scheme),
        ...
        aIsNearer =
          "nearer" === polarity ||
          ("lighter" === polarity && !scheme.isDark) ||
          ("darker" === polarity && scheme.isDark),
      ...
      return (
        ...
        (fTone - nTone) * expansionDir >= delta ||
    ...
    }
    {
      ...
      if (
        ...
        const lightOption = Contrast.lighter(upper, desiredRatio),
          ...
          availables = [];
        -1 !== lightOption && availables.push(lightOption),
          -1 !== darkOption && availables.push(darkOption);
        return DynamicColor.tonePrefersLightForeground(bgTone1) ||
  ...
  if (closestToChroma.chroma < chroma) {
    ...
    for (; closestToChroma.chroma < chroma; ) {
      ...
      Math.abs(potentialSolution.chroma - chroma) <
        Math.abs(closestToChroma.chroma - chroma) &&
...
function performAlbers(prealbers, scheme) {
  ...
  return DynamicColor.tonePrefersLightForeground(prealbers.tone) &&
...
(MaterialDynamicColors.contentAccentToneDelta = 15),
  ...
  (MaterialDynamicColors.onSecondary = DynamicColor.fromPalette({
    ...
    palette: (s) => s.secondaryPalette,
    tone: (s) =>
      isMonochrome(s) ? (s.isDark ? 10 : 100) : s.isDark ? 20 : 100,
  ...
  (MaterialDynamicColors.secondaryContainer = DynamicColor.fromPalette({
    ...
    tone: (s) => {
      ...
      return (
        (answer = performAlbers(s.secondaryPalette.getHct(answer), s)), answer
      );
    },
    isBackground: !0,
    background: (s) => MaterialDynamicColors.highestSurface(s),
...
class TonalPalette {
  ...
  static createKeyColor(hue, chroma) {
    ...
    for (let delta = 1; delta < 50; delta += 1) {
      ...
      hctAddDelta < smallestDelta &&
        ((smallestDelta = hctAddDelta), (smallestDeltaHct = hctAdd));
      const hctSubtract = Hct.from(hue, chroma, 50 - delta),
        hctSubtractDelta = Math.abs(hctSubtract.chroma - chroma);
      hctSubtractDelta < smallestDelta &&
        ((smallestDelta = hctSubtractDelta), (smallestDeltaHct = hctSubtract));
    }
    return smallestDeltaHct;
  }
  tone(tone) {
  ...
  constructor(argb, isContent) {
    ...
    isContent
      ? ((this.a1 = TonalPalette.fromHueAndChroma(hue, chroma)),
        ...
        (this.n1 = TonalPalette.fromHueAndChroma(
          hue,
          Math.min(chroma / 12, 4)
        )),
...
function argbFromHex(hex) {
  ...
  return (
    ...
    ((255 << 24) | ((255 & r) << 16) | ((255 & g) << 8) | (255 & b)) >>> 0
  );
}
