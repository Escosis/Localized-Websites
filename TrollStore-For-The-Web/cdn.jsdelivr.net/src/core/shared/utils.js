...
export function colorThemeCSSProperties(...args) {
  ...
  const shadeTintMdLight = getShadeTintColors(
    colorHexToRgb(light["--f7-md-primary"])
  );
  const shadeTintMdDark = getShadeTintColors(
    colorHexToRgb(dark["--f7-md-primary"])
  );
...
export function colorThemeCSSStyles(colors = {}) {
  ...
  const primary = [
    ...
    ...Object.keys(colors).map(
      (colorName) => `--f7-color-${colorName}: ${colors[colorName]};`
    ),
    `}`,
    `.dark{`,
